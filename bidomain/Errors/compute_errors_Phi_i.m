function [errors]= compute_errors_Phi_i(Dati,femregion,solutions,S,t)
% [errors]= compute_errors_Phi_i(Dati,femregion,solutions,S,t)
%==========================================================================
% Compute L2, H1, DG and L-inf errors for INTRACELLULAR POTENTIAL
%==========================================================================
%    called in main2D.m
%
%    INPUT:
...
for ie=1:ne % loop over elements
    ...
    for k=1:length(w_2D) % loop over quadrature nodes
        ...
        x=pphys_2D(k,1);
        y=pphys_2D(k,2);
        local_exact = 2*eval(Dati.exact_sol_Vm)';
        local_grad_exact = 2*[eval(Dati.grad_exact_Vm_x),eval(Dati.grad_exact_Vm_y)];
