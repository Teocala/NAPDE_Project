function [errors]= compute_errors_w(Dati,femregion,solutions,S,t)
% [errors]= compute_errors_w(Dati,femregion,solutions,S)
%==========================================================================
...
for ie=1:ne % loop over elements
    ...
    for k=1:length(w_2D) % loop over quadrature nodes
        ...
        x=pphys_2D(k,1);
        y=pphys_2D(k,2);
        local_exact=eval(Dati.exact_w)';
        local_grad_exact=[eval(Dati.grad_w_x),eval(Dati.grad_w_y)];
