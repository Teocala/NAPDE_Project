...
if test=='Test1' % Test with physiological parameters 
...
elseif test=='Test2' % Test with physiological FitzHugh-Nagumo parameters  
    DATA = struct( 'name',          test,...
               ... % Test name
               'method',              'SI',...
               ... % Set time discretization (SI, OS, GO)
...
elseif test=='Test3' % Test with unitary parameters
DATA = struct( 'name',             test,...
               ... % Test name
               'method',              'SI',...
               ... % Set time discretization (SI, OS, GO)
...
elseif test=='Test4' % Test with physiological parameters  (well-posed)
    DATA = struct( 'name',          test,...
               ... % Test name
               'method',              'SI',...
               ... % Set time discretization (SI, OS, GO)
               ...
               'theta',             1, ...
               ... % Theta-method ...
               'initialcond_i',      'sin(2*pi*x).*sin(2*pi*y)+0.5', ...
               ... % Initial condition intracellular 
               'initialcond_e',      '0.5*sin(2*pi*x).*sin(2*pi*y)', ...
               ... % Initial condition extracellular
               'exact_sol_i',        'sin(2*pi*x).*sin(2*pi*y).*exp(-5*t)+0.5',...
               ... % Definition of exact solution intracellular
               'exact_sol_e',        '0.5*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t)',...
               ... % Definition of exact solution extracellular
               'exact_sol_Vm',        '0.5*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t)+0.5',...
               ... % Definition of exact solution 
               'source_i',           '(-5*ChiM*Cm + 16*pi*pi*sigma_i)*(0.5*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t))      - kappa*ChiM*(0.5*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t)+0.5).*(0.5*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t) +0.5 - 1).*(0.5*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t) +0.5- a) - ChiM*(epsilon/(epsilon*gamma-5))*(0.5*sin(2*pi*x).*sin(2*pi*y).*exp((-5).*t)+0.5-2.5/(epsilon*gamma))',...
               ... % Forcing term in time intracelluar            
               'source_e',           '(-5*ChiM*Cm - 8*pi*pi*sigma_e)*(0.5*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t))       - kappa*ChiM*(0.5*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t)+0.5).*(0.5*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t) +0.5 - 1).*(0.5*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t) +0.5- a) - ChiM*(epsilon/(epsilon*gamma-5))*(0.5*sin(2*pi*x).*sin(2*pi*y).*exp((-5).*t)+0.5-2.5/(epsilon*gamma))',...    
               ... % Forcing term in time extracellular
               'Neumann_i',           '(sigma_i*(-2*pi*sin(2*pi*x).*cos(2*pi*y).*exp(-5*t).*(y==0) + 2*pi*cos(2*pi*x).*sin(2*pi*y).*exp(-5*t).*(x==1) + 2*pi*sin(2*pi*x).*cos(2*pi*y).*exp(-5*t).*(y==1) -  2*pi*cos(2*pi*x).*sin(2*pi*y).*exp(-5*t).*(x==0)))',...
               ... % Boundary condition intracellular
               'Neumann_e',           '0.5*(sigma_e*(-2*pi*sin(2*pi*x).*cos(2*pi*y).*exp(-5*t).*(y==0) + 2*pi*cos(2*pi*x).*sin(2*pi*y).*exp(-5*t).*(x==1) + 2*pi*sin(2*pi*x).*cos(2*pi*y).*exp(-5*t).*(y==1) -  2*pi*cos(2*pi*x).*sin(2*pi*y).*exp(-5*t).*(x==0)))',...
               ... % Boundary condition extracellular (taken with "-")
               'grad_exact_Vm_x',     'pi*cos(2*pi*x).*sin(2*pi*y).*exp(-5*t)',... 
               ... % Definition of exact gradient Vm wrt x 
               'grad_exact_Vm_y',     'pi*sin(2*pi*x).*cos(2*pi*y).*exp(-5*t)',...    
               ... % Definition of exact gradient Vm wrt y 
               'ChiM',              1e5,...
               ... % Parameter monodomain equation
               'Sigma_i',           0.12,...
               ...
               'a',                13e-3,...
               ... % Parameter ODE 
               'initialw',         '(epsilon/(epsilon*gamma-5))*(0.5*sin(2*pi*x).*sin(2*pi*y)+0.5-2.5/(epsilon*gamma))',...
               ... % Initial condition ODE
               'exact_w',          '(epsilon/(epsilon*gamma-5))*(0.5*sin(2*pi*x).*sin(2*pi*y).*exp((-5).*t)+0.5-2.5/(epsilon*gamma))',...
               ... % Exact solution of ODE 
               'grad_w_x',          '2*pi*(epsilon/(epsilon*gamma-5))*0.5*cos(2*pi*x).*sin(2*pi*y).*exp((-5).*t)',...
                ... % Grad_exact_w_x 
               'grad_w_y',          '2*pi*(epsilon/(epsilon*gamma-5))*0.5*sin(2*pi*x).*cos(2*pi*y).*exp((-5).*t)',...  
                ... % Grad_exact_w_y 
               'fem',               'D1',...   
               ... % Finite element space (choices 'P1,'D1','P2','D2', 'P3','D3')
               'penalty_coeff',     10,... 
...
elseif test=='Test5' % Test with physiological parameters  (strongly well-posed)
    DATA = struct( 'name',          test,...
               ... % Test name
               'method',              'SI',...
               ... % Set time discretization (SI, OS, GO)
               ...
               'theta',             1, ...
               ... % Theta-method ...
               'initialcond_i',      '0.5*sin(2*pi*x).*sin(2*pi*y)+0.5', ...
               ... % Initial condition intracellular 
               'initialcond_e',      '0.25*sin(2*pi*x).*sin(2*pi*y)', ...
               ... % Initial condition extracellular
               'exact_sol_i',        '0.5*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t)+0.5',...
               ... % Definition of exact solution intracellular
               'exact_sol_e',        '0.25*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t)',...
               ... % Definition of exact solution extracellular
               'exact_sol_Vm',        '0.25*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t)+0.5',...
               ... % Definition of exact solution 
               'source_i',           '(-5*ChiM*Cm + 16*pi*pi*sigma_i)*(0.25*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t))      - kappa*ChiM*(0.25*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t)+0.5).*(0.25*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t) +0.5 - 1).*(0.25*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t) +0.5- a) - ChiM*(epsilon/(epsilon*gamma-5))*(0.25*sin(2*pi*x).*sin(2*pi*y).*exp((-5).*t)+0.5-2.5/(epsilon*gamma))',...
               ... % Forcing term in time intracelluar            
               'source_e',           '(-5*ChiM*Cm - 8*pi*pi*sigma_e)*(0.25*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t))       - kappa*ChiM*(0.25*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t)+0.5).*(0.25*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t) +0.5 - 1).*(0.25*sin(2*pi*x).*sin(2*pi*y).*exp(-5*t) +0.5- a) - ChiM*(epsilon/(epsilon*gamma-5))*(0.25*sin(2*pi*x).*sin(2*pi*y).*exp((-5).*t)+0.5-2.5/(epsilon*gamma))',...    
               ... % Forcing term in time extracellular
               'Neumann_i',           '0.5*(sigma_i*(-2*pi*sin(2*pi*x).*cos(2*pi*y).*exp(-5*t).*(y==0) + 2*pi*cos(2*pi*x).*sin(2*pi*y).*exp(-5*t).*(x==1) + 2*pi*sin(2*pi*x).*cos(2*pi*y).*exp(-5*t).*(y==1) -  2*pi*cos(2*pi*x).*sin(2*pi*y).*exp(-5*t).*(x==0)))',...
               ... % Boundary condition intracellular
               'Neumann_e',           '0.25*(sigma_e*(-2*pi*sin(2*pi*x).*cos(2*pi*y).*exp(-5*t).*(y==0) + 2*pi*cos(2*pi*x).*sin(2*pi*y).*exp(-5*t).*(x==1) + 2*pi*sin(2*pi*x).*cos(2*pi*y).*exp(-5*t).*(y==1) -  2*pi*cos(2*pi*x).*sin(2*pi*y).*exp(-5*t).*(x==0)))',...
               ... % Boundary condition extracellular (taken with "-")
               'grad_exact_Vm_x',     '0.5*pi*cos(2*pi*x).*sin(2*pi*y).*exp(-5*t)',... 
               ... % Definition of exact gradient Vm wrt x 
               'grad_exact_Vm_y',     '0.5*pi*sin(2*pi*x).*cos(2*pi*y).*exp(-5*t)',...    
               ... % Definition of exact gradient Vm wrt y 
               'ChiM',              1e5,...
               ... % Parameter monodomain equation
               'Sigma_i',           0.12,...
               ...
               'a',                13e-3,...
               ... % Parameter ODE 
               'initialw',         '(epsilon/(epsilon*gamma-5))*(0.25*sin(2*pi*x).*sin(2*pi*y)+0.5-2.5/(epsilon*gamma))',...
               ... % Initial condition ODE
               'exact_w',          '(epsilon/(epsilon*gamma-5))*(0.25*sin(2*pi*x).*sin(2*pi*y).*exp((-5).*t)+0.5-2.5/(epsilon*gamma))',...
               ... % Exact solution of ODE
               'grad_w_x',          '2*pi*(epsilon/(epsilon*gamma-5))*0.25*cos(2*pi*x).*sin(2*pi*y).*exp((-5).*t)',...
                ... % Grad_exact_w_x 
               'grad_w_y',          '2*pi*(epsilon/(epsilon*gamma-5))*0.25*sin(2*pi*x).*cos(2*pi*y).*exp((-5).*t)',...  
                ... % Grad_exact_w_y 
               'fem',               'D1',...   
               ... % Finite element space (choices 'P1,'D1','P2','D2', 'P3','D3')
               'penalty_coeff',     10,... 
...
elseif test=='Test6' % Test with physiological parameters, coherent with new system signs (+)
DATA = struct( 'name',             test,...
               ... % Test name
               'method',              'SI',...
               ... % Set time discretization (SI, OS, GO)
