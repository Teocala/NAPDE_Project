...
\newpage
\newgeometry{a4paper,top=1cm,bottom=2cm,left=1cm,right=1cm} 
\begin{figure}[h] \captionsetup{size = large} \caption{Test-case 1, missed activation ($I=500 \cdot 10^3$)}
	\centering
...
\newpage
\newgeometry{a4paper,top=1cm,bottom=2cm,left=1cm,right=1cm} 
\begin{figure}[h] \captionsetup{size = large} \caption{Test-case 1, activation ($I = 700 \cdot 10^3$)}
	\centering
	\begin{subfigure}{0.4\textwidth}
...
As expected, even two completely different sets of parameters show a very similar behavior. As expected, we can distinguish two different phenomena:
\begin{itemize}
...
\noindent We certainly state that all these features were expected and reflect the physical phenomenon. In human cardiac electrophysiology is indeed needed a certain current intensity to fully activate the myocyties. Otherwise, cells go soon back to their rest potential. \\

\noindent On the other hand, there are some unexpected mismatches if we look at the potential cycle shown in Fig. (\ref{potential_cycle}). First of all, the rest and activation values should be $-0.09V$ and $0.02V$ and not $0V$ and $1V$. However, it is probably due to the extremely simplicity of the \emph{FitzHugh-Nagumo} model that we adopted. Just from the analytical formulation is clear that $V_m=0$ is an equilibrium value and not $V_m=-0.09$, then a refinement to the model is needed to get these more realistic values. \\
...
\newpage
\newgeometry{a4paper,top=1cm,bottom=2cm,left=1cm,right=1cm} 
\begin{figure}[h] \captionsetup{size = large} \caption{Test-case 2, missed activation ($I = 15000 \cdot 10^3$)} \label{tc2-1}
	\centering
	\begin{subfigure}{0.4\textwidth}
...
\newpage
\newgeometry{a4paper,top=1cm,bottom=2cm,left=1cm,right=1cm} 
\begin{figure}[h] \captionsetup{size = large} \caption{Test-case 2, activation ($I = 20000 \cdot 10^3$)} \label{tc2-2}
	\centering
	\begin{subfigure}{0.4\textwidth}
...
\newpage
\section{Conclusion}
In the end, we have presented two main categories: various comparison for the error analysis (Section (\ref{error_analysis})) and a final realistic simulation (Section (\ref{real_sim})).\\
\noindent From the first one we obtained very good results, indeed any convergence test we have studied was consistent with the theory and the different methods or basis, which we implemented, were equivalent.\\
\noindent Then, for the second group, we got good outcomes, but not perfect because we have not beeen able to achieve repolarization after the activation and so a perfect wave.\\
\noindent Our hope is that this could be a springboard for future projects aiming at improving our results.
\section{Numerical Codes}
\subsection{Implementation: Dubiner to FEM}\label{dub_to_fem}
...
\subsection{Implementation: FEM to Dubiner}\label{fem_to_dub}
...
\end{lstlisting}
\subsection{Implementation: Semi-implicit method}\label{SI}
...
\subsection{Implementation: Godunov operator-splitting method}\label{GO}
...
\subsection{Implementation: Quasi-implicit operator-splitting method}\label{OS}
...
\end{lstlisting}
\subsection{Implementation: imposing a value in a specific point}\label{assign}
...
A(1,1) = 1;
\end{lstlisting}
\subsection{Implementation: imposing zero mean}\label{mean}
