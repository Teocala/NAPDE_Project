...
We observe, before the plateau, a quadratic convergence rate for the error measure in the energy norm and a cubic convergence rates for the error measure in the $L^2$ and $L^\infty$ norms.\\
\paragraph{P3-D3}
\noindent Finally, for polynomials of degree 3, we still find the expected convergence rate except for the two phenomena already discussed in Section \ref{P2-D2}. The results are shown in Fig. \ref{Vm_3}, Fig. \ref{phii_3}, Fig. \ref{phie_3} and Fig. \ref{w_3}. However, because of the third order precision, space errors are smaller and then these effects are amplified. If we do not consider them, we still get the expected orders that are third order for $H^1$ and $DG$ errors and fourth order for $L^2$ and $L^\infty$ norms.\\
...
\begin{center}
	...
	\begin{tabular}{|c|c|} 
		...
		\hline
		\rule[-4mm]{0mm}{1cm}
		$b_i \text{ ($\frac{SV}{{m}^2}$)}$ & 0 \\
		\hline
		\rule[-4mm]{0mm}{1cm}
		$b_e \text{ ($\frac{SV}{{m}^2}$)}$ & 0 \\
		\hline 
		\rule[-4mm]{0mm}{1cm}
		$\chi_m \text{ ($m^{-1}$)}$ & $10^5$ \\
		\hline 
...
\begin{itemize}
	\item $I = 500 \cdot 10^3 $ A as a suitable value for the \emph{underflow} case.
	\item $I = 700 \cdot 10^3 $ A as a suitable value for the \emph{overflow} case.
\end{itemize}
...
\begin{figure}[h] \captionsetup{size = large} \caption{Test-case 1, computed snapshots for underflow case ($I=500 \cdot 10^3$ A)} \label{test1_under}
	\centering
...
\newpage
\newgeometry{a4paper,top=1cm,bottom=2cm,left=1cm,right=1cm} 
\begin{figure}[h] \captionsetup{size = large} \caption{Test-case 1, computed snapshots for overflow case ($I = 700 \cdot 10^3$ A)}\label{test1_over}
	\centering
	\begin{subfigure}{0.4\textwidth}
...
\newpage
\newgeometry{a4paper,top=1cm,bottom=2cm,left=1cm,right=1cm} 
\begin{figure}[h] \captionsetup{size = large} \caption{Test-case 2, computed snapshots for underflow case ($I = 15000 \cdot 10^3$ A)} \label{tc2-1}
	\centering
	\begin{subfigure}{0.4\textwidth}
...
\newpage
\newgeometry{a4paper,top=1cm,bottom=2cm,left=1cm,right=1cm} 
\begin{figure}[h] \captionsetup{size = large} \caption{Test-case 2, computed snapshots for overflow case ($I = 20000 \cdot 10^3$ A)} \label{tc2-2}
	\centering
	\begin{subfigure}{0.4\textwidth}
